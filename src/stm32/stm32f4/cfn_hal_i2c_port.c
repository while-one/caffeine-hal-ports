...
static void low_level_init(cfn_hal_i2c_t *driver)
{
    ...
    /* 2. Initialize Pins */
    if (driver->phy->scl) { (void)cfn_hal_base_init((cfn_hal_driver_t *)driver->phy->scl, CFN_HAL_PERIPHERAL_TYPE_GPIO); }
    if (driver->phy->sda) { (void)cfn_hal_base_init((cfn_hal_driver_t *)driver->phy->sda, CFN_HAL_PERIPHERAL_TYPE_GPIO); }
}
...
static cfn_hal_error_code_t port_i2c_xfr_polling(cfn_hal_i2c_t *driver, const cfn_hal_i2c_transaction_t *xfr, uint32_t timeout)
{
    uint32_t port_id = (uint32_t)(uintptr_t)driver->phy->instance;
    I2C_HandleTypeDef *hi2c = &port_hi2cs[port_id];
    ...
    {
        status = HAL_I2C_Master_Transmit(hi2c, xfr->slave_address << 1, (uint8_t *)xfr->tx_payload, (uint16_t)xfr->nbr_of_tx_bytes, timeout);
        if (status != HAL_OK) { return cfn_hal_stm32_map_error(status); }
    }

    if (xfr->nbr_of_rx_bytes > 0)
    {
        status = HAL_I2C_Master_Receive(hi2c, xfr->slave_address << 1, xfr->rx_payload, (uint16_t)xfr->nbr_of_rx_bytes, timeout);
        if (status != HAL_OK) { return cfn_hal_stm32_map_error(status); }
    }
...
static cfn_hal_error_code_t port_i2c_mem_write(cfn_hal_i2c_t *driver, const cfn_hal_i2c_mem_transaction_t *mem_xfr, uint32_t timeout)
{
    uint32_t port_id = (uint32_t)(uintptr_t)driver->phy->instance;
    uint16_t st_mem_size = (mem_xfr->mem_addr_size == 1) ? I2C_MEMADD_SIZE_8BIT : I2C_MEMADD_SIZE_16BIT;
    return cfn_hal_stm32_map_error(HAL_I2C_Mem_Write(&port_hi2cs[port_id], mem_xfr->dev_addr << 1, mem_xfr->mem_addr, st_mem_size, mem_xfr->data, (uint16_t)mem_xfr->size, timeout));
}

static cfn_hal_error_code_t port_i2c_mem_read(cfn_hal_i2c_t *driver, const cfn_hal_i2c_mem_transaction_t *mem_xfr, uint32_t timeout)
{
    uint32_t port_id = (uint32_t)(uintptr_t)driver->phy->instance;
    uint16_t st_mem_size = (mem_xfr->mem_addr_size == 1) ? I2C_MEMADD_SIZE_8BIT : I2C_MEMADD_SIZE_16BIT;
    return cfn_hal_stm32_map_error(HAL_I2C_Mem_Read(&port_hi2cs[port_id], mem_xfr->dev_addr << 1, mem_xfr->mem_addr, st_mem_size, mem_xfr->data, (uint16_t)mem_xfr->size, timeout));
}

static cfn_hal_error_code_t port_i2c_xfr_irq(cfn_hal_i2c_t *driver, const cfn_hal_i2c_transaction_t *xfr) { CFN_HAL_UNUSED(driver); CFN_HAL_UNUSED(xfr); return CFN_HAL_ERROR_NOT_SUPPORTED; }
static cfn_hal_error_code_t port_i2c_xfr_irq_abort(cfn_hal_i2c_t *driver) { CFN_HAL_UNUSED(driver); return CFN_HAL_ERROR_NOT_SUPPORTED; }
static cfn_hal_error_code_t port_i2c_xfr_dma(cfn_hal_i2c_t *driver, const cfn_hal_i2c_transaction_t *xfr) { CFN_HAL_UNUSED(driver); CFN_HAL_UNUSED(xfr); return CFN_HAL_ERROR_NOT_SUPPORTED; }
